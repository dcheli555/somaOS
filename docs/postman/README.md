...
| File | Role |
|------|------|
| [`soma-os-medication-smoke.postman_collection.json`](./soma-os-medication-smoke.postman_collection.json) | Collection (health + PUT medication) |
| [`soma-os-local.postman_environment.json`](./soma-os-local.postman_environment.json) | Local environment variables (optional) |

After import, choose the **environment** “Soma OS — local dev”.

## Variables

| Variable | Where to set | Notes |
|---------|----------------|-------|
| `clerk_session_jwt` | Collection **Variables** or Environment | Paste **session JWT** only (starts with `eyJ…`). Get it from **`apps/clerk-dev`** (`pnpm --filter @soma-os/clerk-dev dev` → sign in → **Refresh JWT / Copy**). Do not paste the publishable key here. Mark as secret in Postman. |
...
1. API running: **`pnpm --filter @soma-os/api dev`**
