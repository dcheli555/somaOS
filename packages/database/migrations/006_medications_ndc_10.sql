...
-- ndc_11 remains nullable (TEXT, no NOT NULL constraint on soma_ehr.medications).

ALTER TABLE soma_ehr.medications
  ADD COLUMN ndc_10 TEXT;

COMMENT ON COLUMN soma_ehr.medications.ndc_10 IS
  'Optional 10-character NDC segment (excluding 11-digit check digit form in ndc_11).';

COMMENT ON COLUMN soma_ehr.medications.ndc_11 IS
