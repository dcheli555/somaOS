...
COMMENT ON COLUMN soma_ehr.medication_history.prior_version IS
  'Value of soma_ehr.medications.version immediately before this history row; NULL when the event is create (no prior version).';
...
ALTER TABLE soma_ehr.medication_history DISABLE TRIGGER trg_medication_history_prevent_update;

UPDATE soma_ehr.medication_history
...
ALTER TABLE soma_ehr.medication_history ENABLE TRIGGER trg_medication_history_prevent_update;

ALTER TABLE soma_ehr.medication_history
  ALTER COLUMN change_type SET NOT NULL;

ALTER TABLE soma_ehr.medication_history DROP CONSTRAINT IF EXISTS medication_history_change_type_valid;
